...
description: >
  ...
  finished installable package. Do NOT use for skill evaluation (use ocas-critique)
  or variant proposals (use ocas-mentor).
license: MIT
includes:
  - references/**
  - scripts/**
...
- Skill evaluation/scoring (use Mentor or Critique)
...
- Consolidate orphan or duplicate skills into its natural parent
...
**Note on `review-skill` vs `ocas-critique`:** The 3rd-party `review-skill`
(agentskill-sh) provides lightweight quick-check scoring. `ocas-critique` is
the full OCAS quality engine with 6-phase pipeline, batch mode, iteration
loops, and autonomous improvement. Use `ocas-critique` for all OCAS skills.
Use `review-skill` only for quick structural checks on non-OCAS skills. Never
...
- **`## Integrated:` wrapper sections:** when folding content into a parent skill, do NOT wrap it in `## Integrated: <name>` sections. Refactor the content into the parent's existing section structure instead.
...
- **Incorrect Naming**: NEVER create new `ocas-*` skills without explicit user authorization. The `ocas-` prefix is reserved.
...
- **YAML block scalar truncation**: `description: >` and `description: |` block scalars contain embedded newlines. `execute_code` with `content.split('---')` WILL truncate the file at the first `---` inside the block scalar, destroying body content. ocas-custodian lost 57 lines this way in May 2026. Always use `read_file` for exact line ranges, then `patch` with precise old_string. After any frontmatter edit, verify line count, YAML parse, and body start heading.
...
- Critique (`ocas-critique`) — the evaluation complement to Forge's build pipeline
...
|---|---|---|---|
| `forge:journal-scan` | cron | `*/5 * * * *` | Check journal payload fields for VariantProposal and VariantDecision files from Mentor |
...
When a skill's self-update fails or you need to confirm a skill is at the
latest version without mutating it, run `forge.verify-update`: read the local
`metadata.version` from `SKILL.md`, fetch the remote version via
`gh api "repos/{owner}/{repo}/releases/latest"`, and compare. See
`references/builder_workflows.md` for the full verification procedure.
...
(OCAS prefix, author check, duplicate check, default private).

## Skill library sync audit

When asked to audit sync state of all OCAS skills, or when running a scheduled
sync check, use the workflow in `references/sync_audit_procedure.md`. Trigger
phrases: "sync all ocas skills", "audit skill repos", "are all skills pushed?",
"skill library sync status".

## Skill audit

`forge.audit` audits one or more existing OCAS skills for architecture compliance,
applies fixes, and syncs to GitHub. See `references/compliance-audit-checklist.md`
for the full 7-phase procedure and `references/builder_workflows.md` for the
detailed workflow.

## Platform notes

Forge uses the `memory` tool lightly — only for build state during multi-step builds. Core build/validate/critique pipeline works on any platform with `write_file`, `read_file`, and `terminal`.
...
| `references/sync_audit_procedure.md` | Before forge.sync-audit or when asked "are all skills pushed?" |
...
| `references/compliance-audit-checklist.md` | Before auditing an existing skill for OCAS compliance |
| `references/builder_workflows.md` | Before forge.verify-update, forge.consolidate, forge.sync, or forge.audit |
| `references/github_repo_guardrails.md` | Before creating any GitHub repo or PR for a skill |
...
|| `references/frontmatter-editing-pitfalls.md` | Before editing any skill frontmatter; when encountering YAML block scalars |
