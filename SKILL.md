...
- **Dispatch eval-store entries are keyed THREE ways — gap scanners must accept all keys (confirmed live 2026-08-23):** `commons/data/ocas-dispatch/journals_evaluated.jsonl` contains entries keyed `"filename"` (convergence-sweep), `"journal_id"` (bridge_eval_inline/manual), and `"journal"` (other bridge writers). `closure_convergence_sweep.py` and `verify_genuine_gap_profile.py` only loaded `"filename"`, so they re-detected the same gaps forever while `append_unique`'s substring dedup guard blocked re-appending — an infinite BRIDGED-loop with GENUINE GAP never reaching 0. Fix applied 2026-08-23: both scripts now union membership across all three keys. **Second variant, same day:** the sweep itself HISTORICALLY wrote `filename: "journals/<rel>"` (prefix-inclusive) while checking bare relpaths — its own writes never satisfied its own check. Fix applied 2026-08-23: both scripts now run every stored key through `norm_rel()` (strips `journals/`, `commons/journals/`, `profiles/indigo/commons/journals/` prefixes). Any new gap scanner over these stores must do BOTH: union all three keys AND normalize prefixes.
- **PRAXIS eval store key-union bug (fixed 2026-08-24):** `verify_genuine_gap_profile.py` and `closure_convergence_sweep.py` unioned all three keys (`filename`/`journal_id`/`journal`) only for the DISPATCH store; praxis checked `journal_id` alone, so entries written under `"journal"` (e.g. cross_skill_noop bridge lines) re-fired as infinite false gaps — the sweep "re-bridged" the same file forever while the verifier still reported GAP>0. Fix: both scripts now union all three keys for BOTH stores (with norm_rel). Confirmed live: sweep → GAPS BRIDGED: 0, GENUINE GAP: 0, gates ALL CLOSED.
- **FOURTH eval-store key variant `journal_file` (found live 2026-08-24):** some dispatch-store writers key entries `"journal_file"` (e.g. finch-daily noop bridge). After the three-key union above, `closure_convergence_sweep.py` still reported "BRIDGED 3" every iteration while `verify_genuine_gap_profile.py` kept reporting GAP>0 — the sweep's writes landed under keys its own membership check never read, and `bridge_eval_inline.py` skipped because the file was present in the praxis store. Fix applied 2026-08-24: both scanners now union FOUR keys (`filename`, `journal_id`, `journal`, `journal_file`) on BOTH stores; residual taste-scan entry bridged into the dispatch store manually. Any future eval-store writer must either use one of these four keys or update both scanners.
