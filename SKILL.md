...
license: MIT
source: https://github.com/<agent-handle>/forge
includes:
- references/**
- scripts/**
metadata:
...
=======
- **Dispatcher `new_files` paths lack prefix — AND the dispatch-wave journal resolves to the WRONG tree (confirmed 2026-07-16)** — The dispatcher's `details.new_files` prints `ocas-dispatch/2026-07-16/dispatch-wave-20260716T195230Z.json`. Treated as profile-relative, that resolves to `commons/data/ocas-dispatch/<DATE>/` — but that directory holds only LEGACY mirror dispatch-wave JSONs (2026-06 era). The LIVE on-disk journal is at `commons/journals/ocas-dispatch/2026-07-16/dispatch-wave-20260716T195230Z.json`. A `read_file` on the dispatcher-printed path returns "File not found." Route all dispatch-wave journal reads to the `commons/journals/` tree (the same tree `verify_genuine_gap_profile.py` / `closure_convergence_sweep.py` walk). Use `find ~/.hermes -name 'dispatch-wave-*.json'` to locate the real path.
>>>>>>> Stashed changes
...
=======
**Monitor gate state is a SEPARATE file — and it has TWO copies (confirmed 2026-07-16 closure):** the journal re-fire is gated by `monitor_journals.py` reading the PROFILE-relative monitor state at `~/.hermes/profiles/indigo/commons/data/monitor_state/journal_ingest_state.json` (`latest_mtime`) — NOT `ocas-praxis/ingest_state.json`. Closing a re-detection MUST advance BOTH monitor copies AND the praxis `last_ingest_run`, all past the max journal mtime. Always `json.load`+overwrite BOTH copies (`~/.hermes/commons/data/monitor_state/journal_ingest_state.json` and `~/.hermes/profiles/indigo/commons/data/monitor_state/journal_ingest_state.json`). **CORRECTED 2026-07-17:** `scripts/closure_closeout_check.py` (an ocas-FORGE script, lives only in `ocas-forge/scripts/`, NOT ocas-dispatch) was updated to read BOTH monitor copies and to assert gate [2] against the profile copy (the load-bearing one); the old version read only the root copy and gave a false "closed". The verifier's gate [2] reporting True IS now proof the monitor is satisfied (both copies checked). **Email gate fix (2026-07-17):** the verifier no longer requires the two top-level GWS-snapshot files (`last_email_check.json`, `last_email_check_<account-identity>_gmail_com.json`) — those stay `null` under the monitor re-fire bug and are the known un-closeable gate; it now REQUIRES only the dispatch-owned account copies (`owner/last_email_check.json`, `last_email_check_owner.json`, indigo equivalents) and WARNs on the top-level snapshots. See `references/redetection-stale-state-closure-oneshot.md` (Monitor gate state section).
- **Never recursive-glob `commons/journals/**/*.json` during closure DIAGNOSIS (confirmed 2026-07-16):** a symlink/dir loop nests `commons/journals/journals/journals/...` to arbitrary depth, so a recursive `**/` glob over the journals tree returns millions of duplicate paths and can dump 30+ MB into one tool result, burying the real answer. The closure scripts (`closure_convergence_sweep.py`, `verify_genuine_gap_profile.py`) already use a BOUNDED per-skill `os.listdir(skill)/<DATE>` walk — copy that pattern for ad-hoc discovery: glob `commons/journals/<skill>/<DATE>/*.json` per skill, never `**`. **BUT this warning does NOT apply to `run_mixed_wave_closure.py` (verified 2026-07-22):** that runner builds its Mentor file list via `glob("{root}/**/*.json", recursive=True)` over BOTH `~/.hermes/commons/journals` (a symlink → the profile tree) AND `~/.hermes/profiles/indigo/commons/journals` — which resolve to the SAME physical tree (no nested `journals/journals` node exists), so each journal appears twice in the list but it does NOT infinite-loop. The 3-day mtime filter bounds the walk (~21k files, completes <2 min). Ingestion is idempotent by `run_id`. Do NOT abort or rewrite that runner thinking it will hang — the "never recursive-glob" rule targets ad-hoc diagnosis globs that DO nest infinitely, not this bounded runner.
>>>>>>> Stashed changes
...
- **GitHub OCAS repos** — `gh search repos "ocas-* user:<agent-handle>" --json fullName,description,url`
...
> **Closure path-shape + verified re-detection runbook (2026-07-23):** see `references/closure-journal-path-shape-pitfall.md` — the canonical journal path is `commons/journals/<skill>/<DATE>/*.json` (NOT `journals/<DATE>/<skill>/`); a hand-rolled walk over the wrong shape silently returns 0 and the wave re-fires forever. Includes the full 8-step closure sequence + the mandatory email-evidence gate.
...
| `scripts/bridge_explicit_run.py` | **Caller-side bridge for explicit-run `new_journals`-ONLY dispatch waves (no `new_emails`, no `taste_new_data`).** Runs Forge scan + real Mentor heartbeat (subprocess stdin, not shell pipe) + idempotent dual-eval bridge + dispatch-wave journal + state advance in one atomic run. USE THIS INSTEAD of the broken `run_dispatch_pipeline.py` for pure `new_journals` explicit-run overrides. **DO NOT use for MIXED waves** — it omits the Praxis ingest and the taste dedup path and does NOT re-affirm the email second-wave state file. **For a mixed `new_journals`+`new_emails`+`taste_new_data` wave, use `scripts/run_mixed_wave_closure.py`** (the complete closure runner that reaches `=== gates ALL CLOSED ===`). All timestamps composed once; phantom-entry guard via on-disk existence check. **TWO WAVE JOURNALS EXPECTED (confirmed 2026-07-16):** when the email pass runs separately (genuine OR second-wave), it writes its OWN `dispatch-wave-*.json` — so a single mixed dispatcher fire legitimately yields TWO wave journals: one from this script (at the forge-scan TS) and one from the email triage (at its own TS). Both must be bridged into BOTH eval stores (`commons/data/ocas-praxis/journals_evaluated.jsonl` + `commons/data/ocas-dispatch/journals_evaluated.jsonl`). This is NOT a duplication defect and the second journal is NOT a phantom — do not purge it in closure. |
| `scripts/run_mixed_wave_closure.py` | **Complete mixed-wave closure runner** (Forge + Mentor + Praxis + Taste). Added 2026-07-22 to fill the gap `bridge_explicit_run.py` leaves on mixed waves. Composes TS ONCE and reads every pipeline journal relpath from disk (avoids the two documented hand-rolled pitfalls: cross-call timestamp drift -> phantom eval line -> gate [1] False; recomposing `forge-scan-<WAVE_TS>.json` instead of the real on-disk relpath). Usage: `python3 skills/ocas-forge/scripts/run_mixed_wave_closure.py --dispatch-ts <YYYYmmddTHHMMSSZ> --new-files <rel>... --taste-signals-before <N> --taste-delta <M> --apply-taste`. Runs Forge no-op scan, Mentor heartbeat (stdin redirect), Praxis ingest, optional Taste dedup, writes dispatch-wave journal, bridges all (require-exists), convergence sweep, gap assert, email `verified_second_wave` re-affirm, advances gate state, and asserts `=== gates ALL CLOSED ===` (exits non-zero if not). For taste dedup it follows `references/dispatch-wave-taste.md`: verify `signals.jsonl` count already matches `counts.signals` (extraction current) and apply `dispatch_taste_dedup.py` with the 5–8% over-removal tolerance. Replace the hand-rolled `/tmp/run_pipeline_*.py` advice in `references/mixed-wave-closure-one-shot.md` with this script for mixed waves. **SAFE-GLOB NOTE (verified 2026-07-22):** its Mentor file list uses `glob("{root}/**/*.json", recursive=True)` over BOTH `~/.hermes/commons/journals` (symlink → profile tree) and `~/.hermes/profiles/indigo/commons/journals` — same physical tree, so no infinite nesting; bounded by a 3-day mtime filter (~21k files, <2 min). The "never recursive-glob" SKILL.md warning targets ad-hoc diagnosis globs, not this runner. **TWO WAVE JOURNALS EXPECTED (confirmed 2026-07-16):** when the email pass runs separately (genuine OR second-wave), it writes its OWN `dispatch-wave-*.json` — so a single mixed dispatcher fire legitimately yields TWO wave journals: one from this script (at the forge-scan TS) and one from the email triage (at its own TS). Both must be bridged into BOTH eval stores (`commons/data/ocas-praxis/journals_evaluated.jsonl` + `commons/data/ocas-dispatch/journals_evaluated.jsonl`). This is NOT a duplication defect and the second journal is NOT a phantom — do not purge it in closure. **LOAD-BEARING `--date` GOTCHA (confirmed live 2026-07-23):** `--date` defaults to system-UTC `today` and selects the journal DIR the runner writes to AND gap-scans — it is NOT cosmetic like `--dispatch-ts`. When `new_files` include custodian journals whose filename date is ahead of their directory date (the ~7h12m mtime offset — files named `...20260723T00...Z.json` physically living in `ocas-custodian/2026-07-22/`), you MUST pass `--date 2026-07-22` (the DIR date), never the detection date or `today`. Wrong `--date` writes the wave journal into the wrong dir and leaves the custodian gap open → wave re-fires forever. Derive `--date` from `ls -la` of each `new_file`'s parent dir. See `references/run-mixed-wave-closure-date-pitfall.md`. **RESIDUAL-GAP NOTE (confirmed live 2026-07-23):** the runner prints `=== gates ALL CLOSED ===` / `CLOSURE OK` even when a `mentor-light` heartbeat lands mid-run AFTER its single convergence sweep — `verify_genuine_gap_profile.py` can still report `GENUINE GAP > 0`. Gate [2] does not check eval-store membership, so the runner closes on gate-state alone. ALWAYS run a post-run settle pass: `bridge_eval_inline.py --action cross_skill_mitigation --require-exists <leftover file>` then re-run `verify_genuine_gap_profile.py --date <DATE>` to reach `GENUINE GAP (excluding custodian): 0`. See `references/run-mixed-wave-closure-residual-gap.md`. **DISPATCHED new_files BRIDGE GAP (confirmed live 2026-07-25):** the runner's explicit bridge list is `[MENTOR_REL, FORGE_REL, WAVE_REL, TASTE_REL]` — it does NOT include the dispatcher-flagged `--new-files`; those rely on `closure_convergence_sweep.py`, but the sweep can leave a non-custodian dispatched journal (e.g. `ocas-custodian/...lite.json`) present in the PRAXIS eval store yet ABSENT from the DISPATCH eval store while `GENUINE GAP (excluding custodian)` STILL reports 0 and `=== gates ALL CLOSED ===` STILL prints. After the runner returns CLOSURE OK, ALWAYS re-grep each dispatched `new_file` in BOTH `commons/data/ocas-dispatch/journals_evaluated.jsonl` and `commons/data/ocas-praxis/journals_evaluated.jsonl` (bare relpath, no `commons/journals/` prefix); any trace missing from the dispatch store, bridge manually: `python3 scripts/bridge_eval_inline.py <rel> --action mixed_wave_<DATE> --require-exists`. Full recipe + root cause in `references/run-mixed-wave-closure-dispatch-files-bridge-gap.md`. |
| `references/run-mixed-wave-closure-crossdate-newfile-pitfall.md` | **CROSS-DATE new_file gap (confirmed live 2026-07-25):** the runner gap-scans/bridges ONLY the `--date` directory. A dispatched `new_file` in a FORWARD-dated dir (e.g. `ocas-sands/2026-07-26_evening_brief.json`, mtime on dispatch date) is never bridged by the runner and sits 0/0 in both eval stores — `verify_genuine_gap_profile.py --date 2026-07-25` falsely reports `GENUINE GAP (excluding custodian): 0`. After the runner returns CLOSURE OK, manually bridge each cross-date file: `python3 scripts/bridge_eval_inline.py <rel> --action mixed_wave_<DATE> --require-exists`. Distinct from the custodian `--date` pitfall (there the dir is PAST → fix via `--date`; here the dir is FORWARD → keep `--date` and bridge the stray). |
...
=======
| `scripts/closure_closeout_check.py` | **Close-out verifier** (added 2026-07-16, CORRECTED 2026-07-17): asserts ALL closure gates in one pass — named journal in both eval stores, both state gates (`praxis last_ingest_run` + `monitor latest_mtime`) >= max today-journal mtime, and dispatch-owned email `verified_second_wave` flags. Replaces the inline hardcoded close-out heredoc in `references/redetection-stale-state-closure-oneshot.md`. Run `python3 scripts/closure_closeout_check.py --named <rel> --date <DATE>`; exits 1 if any gate is stale. **NOTE this is an ocas-FORGE script — invoked from ocas-forge, NOT ocas-dispatch** (`skills/ocas-dispatch/scripts/closure_closeout_check.py` does NOT exist; confirmed 2026-07-17). **2026-07-17 correction:** (a) reads BOTH monitor copies — root `~/.hermes/commons/data/monitor_state/journal_ingest_state.json` AND profile-relative `~/.hermes/profiles/indigo/commons/data/monitor_state/journal_ingest_state.json` (the profile copy is what `monitor_journals.py` actually gates on; the prior version read only the root copy and falsely reported "closed" while the profile copy stayed stale, re-firing the wave); (b) REQUIRES the dispatch-owned account copies (`owner/last_email_check.json`, `last_email_check_owner.json`, and the indigo equivalents) and only WARNs on the two top-level GWS-snapshot files (`last_email_check.json`, `last_email_check_<account-identity>_gmail_com.json`) which stay `null` under the monitor re-fire bug — the prior version required those and so every <operator> closure failed gate [3] regardless of real state. |
| `scripts/advance_gate_state.py` | **Gate-state advancer** (CREATED + CONFIRMED PRESENT AND EXECUTING 2026-07-22 — a live mixed-dispatch closure ran it and reached `=== gates ALL CLOSED ===`). Recomputes `max(os.path.getmtime(...))` over a BOUNDED per-skill walk of `commons/journals/<skill>/<DATE>/*.json` (both `commons/` roots + profile path). Run AFTER `closure_closeout_check.py` reports gate [2] False, then re-run the verifier and require `=== gates ALL CLOSED ===`. **PITFALL (confirmed live 2026-07-22): requires the `--date YYYY-MM-DD` argument — there is NO default.** Omitting it aborts with `error: the following arguments are required: --date`, which silently leaves gate [2] stale (monitor/praxis state not advanced) and the wave re-fires forever. Always pass `--date <DATE>` matching the closure date. The +5s PAD covers any heartbeat landing during the write; never hand-type the mtime literal (truncation re-fires the wave forever). NOTE: this script uses `os.listdir` per skill-date dir (NOT a `**` recursive glob) — but `run_mixed_wave_closure.py` DOES use `glob(..., recursive=True)` over both journal roots; that is bounded and safe (the two roots are the same symlinked tree, no nested `journals/journals` node), so "symlink loop nests infinitely" applies only to ad-hoc diagnosis globs, not to either shipped runner. Do not reconstruct unless a script truly disappears from disk. |
>>>>>>> Stashed changes
...
=======
| `references/redetection-stale-state-closure-oneshot.md` | **MODE C re-detection stale-state closure — one-shot runbook (2026-07-16):** Date-agnostic copy-paste pre-flight + closure sequence for the PURE stale-state loop (journal already in BOTH eval stores). Covers BOTH `last_ingest_run < named-journal-mtime` (named file is the gap) AND `last_ingest_run == named-journal-mtime` (named file is bridged; LATER mentor-cron heartbeats are the real gap — re-fire is NOT a misclassification). Includes the mandatory POST-ADVANCE re-sweep loop (a mentor-cron heartbeat lands between the pre-advance sweep and the state write), the `json.load`-not-`read_file` guard, the REAL eval-store paths (`commons/data/ocas-*/journals_evaluated.jsonl`, NOT `commons/journals/ocas-dispatch/...`), and the email second-wave re-affirm block (incl. the **account-mislabel pitfall** — a <operator> file carrying `account: <agent-email>` OR `account: null`; the overwrite guard `if s.get("account") != exp_acct` catches BOTH — confirmed 2026-07-16T19:52Z). For the final gate assertion, prefer `scripts/closure_closeout_check.py --named <rel> --date <DATE>` over the inline heredoc. Use this for a plain `new_journals` re-fire that is NOT a mixed wave. |
| `references/mixed-wave-closure-email-evidence-gate.md` | **Mixed-wave closure email-evidence gate (2026-07-17):** The `closure_closeout_check.py` gate [3] `verified_second_wave: True` is NOT proof of email work (2026-07-14 false-positive incident). Before re-affirming/advancing state on ANY mixed wave, run `verify_evidence_threads.py` on every dispatch thread; only close when all print `in_evidence(structured)  action=...`. Full command, authoritative evidence path, and the 2026-07-17 confirmation (10 <operator> threads all genuinely `action=none`). |
>>>>>>> Stashed changes
...
| `references/session-20260722-dispatch-closure.md` | 2026-07-22 closure notes: mixed dispatching, sequential mentor-bridge-verify flow, monitor state advancement, and post-mentor gap discovery pattern |
| `references/session-20260723-dispatch-closure.md` | **Dispatch re-fire closure (2026-07-23):** Verified reproduction of the post-dispatch-gap closure. Dispatcher re-fired an already-processed wave (`dispatch-wave-20260723T042044Z.json`, processed 04:20Z) carrying `new_emails` + `new_journals` — same failure class as the recurring re-fire loop. Diagnosis: wave already in both eval stores; gate [2] stale ONLY because 2 post-dispatch mentor-cron heartbeats (042304Z, 042555Z) landed after the wave and were absent from both stores. Email-evidence gate PASS (`verify_evidence_threads.py` → `in_evidence(structured) action=none`); both heartbeats bridged as `cross_skill_noop_mentor_heartbeat`; convergence sweep 0 additions; `GENUINE GAP (excluding custodian): 0`; `advance_gate_state.py --date 2026-07-23`; `closure_closeout_check.py` → `=== gates ALL CLOSED ===`. Exact command sequence + why `entities_observed: ["ocas-dispatch","ocas-mentor"]` still counts as a noop heartbeat. |
| `references/session-20260724-dispatch-closure.md` | **Dispatch re-fire closure (2026-07-24):** Another re-detection of an already-closed mixed wave (`dispatch-wave-20260724T072118Z`). Both `new_files` journals already in both eval stores; email already triaged (Docusign `escalate` PRESERVED, Roller shade `none`); single genuine gap = a post-dispatch noop mentor-light heartbeat (`mentor-light-20260724T072518Z.json`, mtime 1784877920.04, `gap_detected:false`) bridged via `bridge_eval_inline.py` with tag `cross_skill_noop_mentor_heartbeat`; gate state advanced to max mtime +5s into BOTH monitor copies + praxis `ingest_state.last_ingest_run`. `closure_closeout_check.py` → `=== gates ALL CLOSED ===`; `verify_genuine_gap_profile.py --date 2026-07-24` → `GENUINE GAP (excluding custodian): 0`. Re-confirms the journals path-shape trap (`roots/<skill>/<DATE>/`, NOT `roots/<DATE>/<skill>/`) that silently returns 0 files and falsely implies "all closed," and the still-live manual-closure stopgap for the monitor re-fire bug. |
| `references/session-20260724-mixed-closure-minted-wave.md` | **Mixed re-detection closure with MINTED wave journal + 1 Path B (2026-07-24):** when a closure mints a NEW wave journal (because there's genuine Path B work), that journal itself must be bridged into BOTH eval stores or `closure_closeout_check.py` gate [1] fails. Also covers cross-store journal gaps (present in praxis eval store, absent from dispatch eval store) and re-advancing gate state when a post-dispatch heartbeat lands between the first advance and the verifier. |
| `references/session-20260725-continuous-feed-closure.md` | **Continuous-feed dispatch re-fire closure (2026-07-25):** genuine-gap residual was a CONTINUOUS ocas-reach live feed (~12 journals/min, never settles) — not the mentor-heartbeat tail. Run `run_mixed_wave_closure.py`, then LOOP `closure_convergence_sweep.py` → `advance_gate_state.py` → `closure_closeout_check.py` because the feed writes new tail journals after each advance; accept residual few carry to next wave. Pitfall: verify monitor gate state with absolute paths / `closure_closeout_check.py` [2] only — a relative `cat` of `profiles/indigo/commons/...` hits a STALE mirror. |
| `references/session-20260725-emailplusjournal-redetection-nomint.md` | **E-mail + journal COMBINED re-detection, ZERO journals minted (2026-07-25):** a dispatcher fire carried BOTH `new_emails` AND `new_journals` for the SAME already-closed wave. Correct close = email pass appends evidence only (NO dispatch-wave journal) + journal pass bridges residual + advances gate (NO dispatch-wave journal). DO NOT call `run_mixed_wave_closure.py` — it mints a wave journal and re-fires. Distinguishes full no-mint re-detection from the other closure modes. |
| `ocas-dispatch: references/mixed-redetection-mint-vs-nomint.md` | **MINT-vs-NO-MINT decision rule for mixed (email+journal) re-detection (cross-skill ref).** The no-mint session above is the TRUE NO-MINT case only because its `new_files` was the already-closed wave journal itself (already in both eval stores). When a mixed re-detection's `new_files` contains a GENUINELY-NEW journal (not in eval stores, e.g. a routine `mentor-light` heartbeat), you MUST mint + bridge a v2 wave journal or gate [1] fails (the pure-email Path-A no-journal exception does NOT apply to mixed waves). The decision key is the `new_files` eval status, never the email Path-A verdict. Full rule + both worked examples. |
| `references/session-20260725-redetection-verify-and-bridge.md` | **Concrete verify-and-bridge SOP for the no-mint re-detection pattern above:** exact `verify_evidence_threads.py` email-gate command, bare-relpath eval-store grep, the "gate advanced past an UNBRIDGED journal" trap, noop-heartbeat bridge command, and the sweep→verify→advance→re-verify closure sequence. Read this to actually RUN the closure, not just understand it. |
| `references/dispatch-recovery-stale-state-postgap-closure.md` | **Stale-state re-detection with post-dispatch gap journal closure (2026-07-22):** When verifier reports a post-bridge `GENUINE GAP > 0` because an additional post-dispatch mentor-light journal landed after detection, inspect it. **Noop criterion (generalized, confirmed 2026-07-23):** a post-dispatch mentor-light is a safe noop-bridge IFF it carries NO `gap_detected` / `evaluated_count` / gap-evaluation fields — regardless of whether `entities_observed` is exclusively `["ocas-mentor"]` (it may ALSO list `ocas-dispatch` or other skills and still be a pure heartbeat). Bridge such journals with a tag like `cross_skill_noop_mentor_heartbeat` (the `--action` value is a free-form label, not a magic string) WITHOUT rerunning Forge/Mentor/Praxis. A mentor-light that DOES carry `gap_detected: true` or an evaluated-event list is a genuine gap and must run the real pipeline. After bridging noops, re-sweep verifier, advance state past max mtime, close. Reproduced end-to-end 2026-07-23 — see `references/session-20260723-dispatch-closure.md`. |
...
=======
- **Closure script does NOT advance the monitor `latest_mtime` copies (CONFIRMED LIVE 2026-07-17):** `dispatch_redetection_close.py` advances ONLY `ingest_state.last_ingest_run` (the praxis copy). It does NOT touch the two monitor `journal_ingest_state.json` copies (`~/.hermes/commons/data/monitor_state/` root + `~/.hermes/profiles/indigo/commons/data/monitor_state/` profile-relative). Gate [2] in `closure_closeout_check.py` reads BOTH monitor copies, so after the closure script runs, gate [2] stays False and the wave re-fires forever. MANDATORY extra step after any closure run: recompute `max_mt = max(os.path.getmtime(p) for p in glob of ALL today-journals EXCLUDING ocas-custodian + dispatch-wave-*)`, set `new = max_mt + 2.0` (≥2s pad, NEVER hand-typed literal — see `redetection-mtime-truncation-pitfall.md`), and `json.load`+overwrite BOTH monitor copies with `latest_mtime = new`. Then re-run `closure_closeout_check.py` and confirm `=== gates ALL CLOSED ===`. This applies even when you ran the genuine pipeline via `bridge_explicit_run.py` (which also leaves monitor copies stale). CONFIRMED LIVE 2026-07-17: a genuine-run wave (bridge_explicit_run.py + dispatch_redetection_close.py) left gate [2] False (monitor copies at 06:40:41Z vs max journal 06:48:43Z) until both copies were advanced by hand.
>>>>>>> Stashed changes
...
- **GAP-SCAN PARSE PITFALL (observed live 2026-07-22T1206Z):** `verify_genuine_gap_profile.py --date <DATE>` emits two line shapes — per-gap `GAP  <relpath> dispatch=False praxis=True` and a trailing summary `GENUINE GAP (excluding custodian): N`. A hand-rolled bridge loop that filters `startswith("GENUINE")` extracts ONLY the summary COUNT string (never a path), passes it to `bridge_eval_inline.py`, bridges nothing, and the wave never reaches `=== gates ALL CLOSED ===`. Parse the `GAP <relpath>` lines (first token after `GAP  `, skip `custodian=` lines) — or, better, just run `python3 scripts/closure_convergence_sweep.py --date <DATE>` which performs the iterate-and-bridge loop internally and is the documented primitive. Always re-run the verifier afterward and require the literal `GENUINE GAP (excluding custodian): 0` line; the bridge script's own `total bridged: N` printout is NOT the gate. See `references/closure-gap-scan-parse-pitfall.md`.
...
=======
| `references/closure-post-ingest-mtime-trap.md` | **Post-ingest caller-journal mtime trap (2026-07-17):** the Praxis ingest template sets `last_ingest_run` to its OWN NOW mid-pipeline, so caller-written `praxis-cron-*.json` + `dispatch-wave-*.json` (later mtimes) exceed it — verifier gate [2] praxis-half goes False even when `GENUINE GAP=0`, re-firing forever. After writing those journals, RE-ADVANCE `last_ingest_run` past max today-journal mtime (+>=5s pad), then re-sweep/verify/closeout. Distinct cause from the hand-typed truncation trap above. |
| `references/closure-gap-scan-parse-pitfall.md` | **Gap-scan parse pitfall (2026-07-22):** `verify_genuine_gap_profile.py` prints genuine gaps as `GAP  <relpath> dispatch=False praxis=True` and a trailing SUMMARY `GENUINE GAP (excluding custodian): N`. A hand-rolled bridge loop that parses `startswith("GENUINE")` extracts only the COUNT string (no path) and silently bridges nothing — the wave never closes. Parse `GAP <relpath>` lines (first token after `GAP  `, exclude `custodian=`), or just use `closure_convergence_sweep.py --date <DATE>` which iterates bridging internally. |
| `references/closure-praxis-state-iso-format-pitfall.md` | **Praxis `last_ingest_run` ISO vs monitor `latest_mtime` float trap (2026-07-24):** When advancing gate state during closure, the two store types use DIFFERENT field formats — `ocas-praxis/ingest_state.json:last_ingest_run` MUST be an ISO-8601 **string** (`2026-07-24T15:10:25.630054+00:00`), while BOTH `monitor_state/journal_ingest_state.json:latest_mtime` copies are **float epochs**. `closure_closeout_check.py` gate [2] reads praxis via `datetime.fromisoformat(last_ingest_run)` (a float → parse error → `MISSING` → "gates STALE") and monitor via `datetime.fromtimestamp(latest_mtime)` (float OK). Always write ISO to praxis, float to monitor. A float written to praxis makes gate [2] print `False (MISSING ...)` even when numerically correct — re-run with ISO. |
>>>>>>> Stashed changes
