...
- **`skill_view`/`search_files` can fail with a transient backend API error in cron — fall back to reading SKILL.md from disk (confirmed 2026-07-15):** During a live dispatch run both `skill_view(name=...)` and `search_files` repeatedly returned `Error during OpenAI-compatible API call #N: 'DaemonThreadPoolExecutor' object has no attribute '_initializer'` (a fault in the tool's own LLM path) while `terminal()` and `read_file()` kept working. An autonomous dispatch must NOT abort because the skill-read failed. **Fix:** if `skill_view`/`search_files` error, locate and read the source directly — `find <hermes-home>/skills -name SKILL.md | grep <name>` then `read_file` (or `cat` via `terminal()`) — and proceed from the on-disk SKILL.md + `scripts/`. The pipeline is fully runnable without `skill_view`; the verified `references/mixed-wave-closure-one-shot.md` runbook is a plain file you can `read_file` too. Treat this as a retry/fallback path, NOT a permanent "tool is broken" conclusion — the backend error is likely transient (it cleared mid-session).
