...
grep -c "MENTOR_REL" ~/.hermes/profiles/indigo/commons/data/ocas-praxis/journals_evaluated.jsonl
# Check dispatch eval store
grep -c "MENTOR_REL" ~/.hermes/profiles/indigo/commons/data/ocas-dispatch/journals_evaluated.jsonl
```
Use bare relpath (NO `commons/journals/` prefix). If missing, bridge manually:
```bash
python3 -c "
import json
eval_path = '~/.hermes/profiles/indigo/commons/data/ocas-dispatch/journals_evaluated.jsonl'
...
python3 -c "
import os, glob
max_mtime = max(os.path.getmtime(p) for p in glob.glob('~/.hermes/profiles/indigo/commons/journals/*/2026-07-27/*.json'))
...
for f in ~/.hermes/profiles/indigo/commons/data/ocas-dispatch/last_email_check_<operator-email-slug>.json \
         ~/.hermes/profiles/indigo/commons/data/ocas-dispatch/last_email_check_indigo.json \
         ~/.hermes/profiles/indigo/commons/data/ocas-dispatch/last_email_check.json \
         ~/.hermes/profiles/indigo/commons/data/ocas-dispatch/last_email_check_mx_indigo_karasu_gmail_com.json; do
