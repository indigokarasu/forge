...
When writing JSON to eval files via `terminal()`, avoid embedding path variables in `printf` content that matches the redirect target. If `$VAR` is set to `~/.../ocas-praxis/file` and then used in `printf '...' >> "$VAR"`, bash can lose track of variable boundaries in complex multi-statement calls, truncating the path (e.g., `~/.../ocas-`).
